...
def evaluate_on_testset(model, tokenizer, test_file: str) -> dict:
    ...
    for sample in samples:
        ...
        ctx_empty_ok = gt_ctx == "" and _normalize(pred_ctx) in ("", "khong co", "none", "n/a")
    ...
    status = "DAT" if results["meets_target"] else "CHUA DAT"
...
def train(cfg: argparse.Namespace):
    ...
    print("\n[sft_intent] Bat dau training...")
    trainer.train()

    trainer.save_model(cfg.output_dir)
    tokenizer.save_pretrained(cfg.output_dir)
    print(f"[sft_intent] LoRA adapter da luu: {cfg.output_dir}")

    print("\n[sft_intent] Merging LoRA vao base weights...")
    ...
    print(f"[sft_intent] Merged model da luu: {cfg.merged_dir}")

    if Path(cfg.test_file).exists():
        print(f"\n[sft_intent] Danh gia tren test set: {cfg.test_file}")
        ...
        print(f"[sft_intent] Metrics luu: {out_path}")
    else:
        print(f"[sft_intent] Test file khong ton tai, bo qua: {cfg.test_file}")
